                ...
                var sf_batchMerge = function () {
                    ...
                    $('.sf_admin_row').each(function (k, row) {
                        ...
                        if (chb.is(':checked')) {
                            ...
                            chb.parent().css("position", "relative");
                            chb.parent().append('<label class="custom_input_label" style="position:absolute;left:13px;top:10px;"><input onclick="sf_batchMergeCheckRows()" class="batch_primary_id custom_input"' + (!sf_batchMergeChecked ? ' checked="checked"' : '') + ' type="radio" name="batch_primary_id" value="' + chb.val() + '" /><span class="custom_input custom_input_radio"></span></label>');
