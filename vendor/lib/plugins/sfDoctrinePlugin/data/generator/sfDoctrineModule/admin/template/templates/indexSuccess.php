...
<?php if ($this->configuration->hasTabs()): ?>
    <div class="sf_admin_tabs">
        ...
        {
        echo '<a href="' . (isset($tab_val['url_for']) ? url_for($tab_val['url_for']) : '?tab=' . $tab_key) . ($sf_request->hasParameter('q') && $sf_request->getParameter('q') != '' ? '&q=' . $sf_request->getParameter('q') : '') . '" class="sf_admin_tab' . ($tab_key == $current_tab ? ' sf_admin_tab__active' : '') . '">' . $tab_val['label'] . '</a>';
        }
        ?]
    </div>
<?php endif; ?>
...
<div class="batch_form__class">
    ...
    <form id="batch_form" action="[?php echo url_for('<?php echo $this->getUrlForAction('collection') ?>', array('action' => 'batch')) ?]" method="post">
        ...
        <div id="lui_scroller" class="lui_scroller">
            <div class="lui__scroller_wrapper">[?php include_partial('<?php echo $this->getModuleName() ?>/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper)) ?]</div>
        </div>
        ...
        $(document).ready(function () {
            $('#<?php echo $this->getModuleName() ?>_live_search').keyup(function (event) {
                ...
                if ($(this).val().trim() != '' && event.which == 13) {
                    is_not_good_style_of_code('?q=' + encodeURI(jqxhr_old) + (checked.length > 0 ? '&checked=' + checked.join(':') : ''));
