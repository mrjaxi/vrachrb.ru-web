...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        foreach ($apps as $app) {
            ...
            $u = "?u=" . (getenv('CI_COMMIT_SHA') ? getenv('CI_COMMIT_SHA') : time());
            $view_file = sfConfig::get('sf_apps_dir') . '/' . $app . '/config/view.yml';
            if (file_exists($view_file . '.skeleton')) {
                $params = $parser->parse(file_get_contents($view_file . '.skeleton'));
                foreach ($params['default']['stylesheets'] as $stylesheet_k => $stylesheet) {
                    $params['default']['stylesheets'][$stylesheet_k] = $stylesheet . $u;
                }
                foreach ($params['default']['javascripts'] as $javascript_k => $javascript) {
                    $params['default']['javascripts'][$javascript_k] = $javascript . $u;
                }
                file_put_contents($view_file, trim(trim($dumper->dump($params), "{}")));
            }
        ...
        if (null === $options['app'] && 'all' == $options['type']) {
            $this->getFilesystem()->remove(sfFinder::type('file')->discard('.sf')->discard('.gitkeep')->in(sfConfig::get('sf_cache_dir')));
        }
    }
    ...
    protected function clearConfigCache(sfApplicationConfiguration $appConfiguration)
    {
        ...
        if (is_dir($subDir)) {
            // remove cache files
            $this->getFilesystem()->remove(sfFinder::type('file')->discard('.sf')->discard('.gitkeep')->in($subDir));
        }
    }
