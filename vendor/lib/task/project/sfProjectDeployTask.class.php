...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $filename = sfConfig::get('sf_apps_dir') . '/www/config/view.yml';
    $parser = new sfYamlParser($filename);
    $values = $parser->parse(file_get_contents($filename));
    foreach($values['default']['stylesheets'] as $css_k => $css)
    {
      $ex = explode('?', $css);
      $values['default']['stylesheets'][$css_k] = $ex[0] . '?deploy=' . time();
    }
    foreach($values['default']['javascripts'] as $js_k => $js)
    {
      $ex = explode('?', $js);
      $values['default']['javascripts'][$js_k] = $ex[0] . '?deploy=' . time();
    }
