...
var convertImgToBase64 = function (url, callback, _this){
  $.get(sf_prefix + '/proxy?url=' + url, function(base64){
    callback(base64, url, _this);
  });
};
var http2base64Callback = function(base64, url, _this){
  var html = _this.getCodeEditor();
  var fn = basename(url);
  var re = new RegExp('<img src="' + http2base64Placeholder + '" data-file="' + fn, 'gi');
  var mime_ext = 'jpeg';
  var ext = fn.split('.');
  if(ext[1] == 'png' || ext[1] == 'gif'){
    mime_ext = ext[1];
  }
  html = html.replace(re, '<img src="data:image/' + mime_ext + ';base64,' + base64 + '"');
  _this.setCodeEditor(html);
};
var http2base64Placeholder = 'data:image/gif;base64,R0lGODlhAQABAIABAAAAAP///yH5BAEAAAEALAAAAAABAAEAAAICTAEAOw==';
var http2base64 = function(s, url, _this){
  convertImgToBase64('http' + url, function(img, dataURL){http2base64Callback(img, dataURL, _this);}, _this);
  return '<img src="' + http2base64Placeholder + '" data-file="' + basename(url) + '"';
};
...
(function($){
  ...
  function Construct(el, options) 
  {
    this.opts = $.extend({  
      ...
      imageUpload: '/arm/uploader?key=redactor', // url or false
      imageGetJson: '/folder/images.json', // url (ex. /folder/images.json ) or false
      imageUploadFunction: false, // callback function
      
      fileUpload: '/arm/upload_file?key=redactor',
  ...
  Construct.prototype = {
    ...
    {    
      ...
      frameHtml += '<html><head><link media="all" type="text/css" href="' + this.opts.css + '" rel="stylesheet"><link media="all" type="text/css" href="/css/fonts.css" rel="stylesheet"><style type="text/css">img{height:100px;}</style></head>';
      frameHtml += '<body>';
      frameHtml += html;
      frameHtml += '</body></html>';
      return frameHtml;      
    ...
    clean: function(marker)
    {
      ...
      html = html.replace(/<img src="(.*?)">([\w\W]*?)<\/a>/gi, '');
      
      var _this = this;
      html = html.replace(/<img src="http(.*?)"/gi, function(s, url){return http2base64(s, url, _this);});
    ...
    {    
      return html.replace(/<(?!\s*\/?(div|span|label|a|br|p|b|i|del|strike|img|video|audio|iframe|object|embed|param|blockquote|mark|cite|small|ul|ol|li|hr|dl|dt|dd|sup|sub|big|pre|code|figure|figcaption|strong|em|table|tr|td|th|tbody|thead|tfoot|h1|h2|h3|h4|h5|h6)\b)[^>]+>/gi,"");
    },  
    ...
    buildToolbar: function()
    {
      ...
      $.each(RTOOLBAR, function(key,s)
      {
        ...
        var a = $('<a href="javascript:void(null);" style="' + (s.html ? 'width:auto !important;' : '') + '" title="' + s.title + '" class="redactor_btn_' + key + (s.html ? ' redactor_btn_htmlc' : '') + '"><span style="font-size: 13px;padding: 5px;line-height: 13px;width:auto !important;">' + (s.html ? s.html : '&nbsp;') + '</span></a>');
    ...
    */        
    ...
    showFile: function()
    {
      ...
      redactorActive = this;
            ...
            redactorActive = this;
      this.modalInit(RLANG.file, this.opts.path + 'plugins/file.html?r', 500, 310, handler);
    },  
    ...
    fileDownload: function(el, file_id)
    {
      ...
      redactorActive = this;      
            this.modalInit(RLANG.image, this.opts.path + 'plugins/image_edit.html', 395, 180, handler);
        },
    ...
    showVideo: function()
    {
      ...
      redactorActive = this;
      this.modalInit(RLANG.video, this.opts.path + 'plugins/video.html', 600, 300, function()
      {
